...
SELECT 
    ...
    NVL((SELECT SUM(amount) FROM ledger_entries WHERE account_id = a.id AND direction = 'DEBIT'), 0) as total_debits,
    NVL((SELECT SUM(amount) FROM ledger_entries WHERE account_id = a.id AND direction = 'CREDIT'), 0) as total_credits
FROM accounts a;
